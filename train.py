...
def get_data(size, woof, bs, workers=None):
 ...
 #   elif size<=224: path = URLs.IMAGEWOOF_320 if woof else URLs.IMAGENETTE_320
   # else          : 
    if woof:
        path = "https://s3.amazonaws.com/fast-ai-imageclas/imagewoof2.tgz"    # if woof 
    else:
        path = URLs.IMAGENETTE
    # path = untar_data(path)
    path = "/home/lbl/code/DL/Ranger-Mish-ImageWoof-5-master/imagewoof2/val"

    n_gpus = num_distrib() or 1
    if workers is None: workers = min(8, num_cpus()//n_gpus)

    return (ImageDataLoaders.from_folder(path).split_by_folder(valid='val')
